...
@torch.no_grad()
def traj_collecting(model,env,teacher,normalizedEnv,counter,writer,device,args,q_model,q_traj):
...
def get_epoch_trajectories_ursina(model,env,teacher,normalizedEnv,counter,writer,device,args,q_model,q_traj):
    ...
    while True:
        ...
        traj_collecting(model,env,teacher,normalizedEnv,counter,writer,device,args,q_model,q_traj)
...
def ppo_update(device,ppo_epochs, mini_batch_size, text_inputs,actions_length,actions_direction,actions_length_log_probs ,actions_directions_log_probs, returns,advantages,action_masks,values,hidden_states,cell_states,counter,optimizer,model,writer,args ,clip_param=0.1, max_grad_norm = 0.5):
