...
class ALPGMM():
    def __init__(self, mins, maxs, seed=None, params=dict()):
        ...
        #self.random_task_generator = np.array([np.float32(random.uniform(x,y)) for x,y in zip(self.mins,self.maxs)])#Box(self.mins, self.maxs, dtype=np.float32)
        ...
        if(self.load):
            file_to_read = open("GMM", "rb")
            old_bk = pickle.load(file_to_read)
            # Init GMMs
            self.potential_gmms = [self.init_gmm(k) for k,w,m in zip(self.potential_ks,old_bk['weights'],old_bk['means'])]
        else:
            # Init GMMs
        ...
        self.gmm = None
        # Boring book-keeping
        self.bk = {'weights': [[] for i in range(self.potential_ks.shape[0])], 'covariances': [[] for i in range(self.potential_ks.shape[0])], 'means': [[] for i in range(self.potential_ks.shape[0])], 'precisions': [[] for i in range(self.potential_ks.shape[0])], 'tasks_alps': [], 'episodes': []}#{'weights': [[]], 'covariances': [[]], 'means': [[]], 'tasks_alps': [], 'episodes': []}

    def init_gmm(self, nb_gaussians,weights=None, means=None,precisions = None):
    ...
    def update(self, task, reward):
        ...
        alp = self.alp_computer.compute_alp(task, reward)
        self.alps.append(alp)


        # Concatenate task vector with ALP dimension
        self.tasks_alps.append(np.array(task.tolist() + [alp],dtype=float))
        ...
        if len(self.tasks) >= self.nb_random:  # If initial bootstrapping is done
            if (len(self.tasks) % self.fit_rate) == 0:  # Time to fit
                ...
                # book-keeping
                for index,value in enumerate(self.potential_gmms):
                    self.bk['weights'][index].append(value.weights_.copy())
                    self.bk['covariances'][index].append(value.covariances_.copy())
                    self.bk['means'][index].append(value.means_.copy())
                    self.bk['precisions'][index].append(value.precisions_.copy())
                self.bk['tasks_alps'] = self.tasks_alps
                self.bk['episodes'].append(len(self.tasks))

    def sample_task(self):
        if (len(self.tasks) < self.nb_random) or (np.random.random() < self.random_task_ratio) or self.gmm == None:
            # Random task sampling
            new_task = np.array([np.float32(random.uniform(x,y)) for x,y in zip(self.mins,self.maxs)])#self.random_task_generator#.sample()
        else:
